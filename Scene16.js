...
this.add.bitmapText(350, 21, 'pixel', 'SELECT CHARACTER',50).setTint(0xffffff);
this.selecsound = this.sound.add('character');
...
.on('pointerdown', () => {this.scene.start('seleclevel'),this.selecsound.play()});
var jugartex= this.add.bitmapText(510,300, 'pixel', 'PLAY', 60).setTint(0x21911d);
noju= this.add.image(595, 621,'madera').setScale(0.3,1.3).setInteractive()
.on('pointerdown', () => {this.scene.start('selection'),this.selecsound.play()});
var nojutex= this.add.bitmapText(510,600, 'pixel', 'BACK', 60).setTint(0xd10d06);
;} );
...
this.add.image(30,15,'fatras').setScale(0.55).setOrigin(0,0)
    .setInteractive().on('pointerdown', () =>{ this.selecsound.play(), this.scene.start('start')});
