class Scene18 extends Phaser.Scene{
    ...
    create(){
        ...
        instruccionestext5 = this.add.bitmapText(150,750, 'pixel', '- Press the escape key or click the pause button' + 
        '\n    to stop the game.', 30).setTint(0x000000)
        bot4 = this.add.image(1200, 600,'next').setInteractive().on('pointerdown', ()=>{this.physics.resume(); pausacont = 0; instruccionestext.destroy();this.selecsound.play(); instrucciones4.destroy(); instruccionestext5.destroy();logo.destroy(); logo2.destroy();bot4.destroy();
      ...
      .setInteractive().on('pointerdown', ()=>{if(sonion ===0){ game.sound.mute = true;sonion = 1;}
        ...
        //// SCORE
        scoreText = this.add
        .bitmapText(450, 48, "pixel", "SCORE: 0", 50)
        .setTint(0x000000);
        ...
        instruccionestext2 = this.add.bitmapText(125, 750, 'pixel', '- Press space key or the' + 
        ' on-screen button to jump.', 30).setTint(0x000000)
        ...
        instruccionestext3 = this.add.bitmapText(35,750, 'pixel', '- Combine sanitizers to get more points and gain more speed.', 30).setTint(0x000000)
        ...
        instruccionestext = this.add.bitmapText(330, 322, 'pixel', 'Instructions Level 3', 40).setTint(0xffffff)
        instruccionestext4 = this.add.bitmapText(50,720, 'pixel', '- Click on the viruses to eliminate them and score points.', 30).setTint(0x000000)
        ...
        infotext = this.add.bitmapText(470, 322, 'pixel', 'Prevention', 40).setTint(0xffffff)
        logo = this.add.bitmapText(380, 225, 'pixel', 'Flu Heroes', 70).setTint(0xF00909)
        logo2 = this.add.bitmapText(385, 220, 'pixel', 'Flu Heroes', 70).setTint(0xE7BB09)
        infotext4 = this.add.bitmapText(30,640, 'pixel', '\n• Wash your hands frequently or use sanitizer.'+
        ...
        bot5 = this.add.image(1200, 600,'next').setInteractive().on('pointerdown', ()=>{info1.destroy(), infotext.destroy();this.selecsound.play(); bot5.destroy(); infotext4.destroy();
        }
        );
    }
    ...
    puntos = puntos+100;
    if (100 >= puntos <= 190){
        ve = this.add.bitmapText(620,500,'pixel', 'Go to the hospital!').setTint(0xFFFFFF);
        }
    else {
        ve.destroy();
    }
    ...
    pausa(){
        ...
        var t1 = this.add.bitmapText(player.x-140,360, 'pixel', 'Resume', 60).setDepth(5.2).setTint(0x21911d);
        ...
        .setInteractive().on('pointerdown', ()=> {this.selecsound.play(), this.physics.resume(); reanudartext.destroy(); pausa.destroy(); pausa2.destroy(); t1.destroy(); t2.destroy();t3.destroy();
            pausacont = 0; restartButton.destroy(); menui.destroy();})
        var t2 = this.add.bitmapText(player.x-155, 530, 'pixel','Restart', 60).setDepth(5.2).setTint(0x63370a);
        ...
        var t3 =this.add.bitmapText(player.x-90, 680, 'pixel','Exit', 60).setDepth(5.2).setTint(0xecd313);
        menui = this.add.image(player.x-40, 705,'madera').setAlpha(0.000000000001).setScale(0.3,1)
