...
this.add.bitmapText(370, 21, 'pixel', 'LEVEL SELECTION',50).setTint(0xffffff);
nivel1 = this.add.image(250, 362,'madera').setAlpha(0.001).setScale(0.28,5.8).setInteractive()
.on('pointerdown', () => {this.scene.start('juego4'),this.selecsound.play(), reiniciarcont =0} );
...
if (plevel1 > 0){
    ...
    .on('pointerdown', () => {this.scene.start('juego5'),this.selecsound.play(), reiniciarcont =1} );
}
...
if (plevel2 > 0){
    ...
    .on('pointerdown', () => {this.scene.start('juego6'),this.selecsound.play(),  reiniciarcont =2} );
}

this.add.image(30,15,'fatras').setScale(0.55).setOrigin(0,0)
    .setInteractive().on('pointerdown', () =>{this.selecsound.play(), this.scene.start('selection')});
