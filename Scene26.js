...
this.add.bitmapText(240, 21, 'pixel', 'SELECIONE O PERSONAGEM',50).setTint(0xffffff);
this.selecsound = this.sound.add('character');
...
jugar= this.add.image(595, 321,'madera').setScale(0.3,0.99).setInteractive()
.on('pointerdown', () => {this.scene.start('selecpalco'),this.selecsound.play()});
var jugartex= this.add.bitmapText(480,300, 'pixel', 'JOGAR', 60).setTint(0x21911d);
noju= this.add.image(595, 621,'madera').setScale(0.3,0.99).setInteractive()
.on('pointerdown', () => {this.scene.start('selecao'),this.selecsound.play()});
...
this.add.image(30,15,'fatras').setScale(0.55).setOrigin(0,0)
    .setInteractive().on('pointerdown', () =>{ this.selecsound.play(), this.scene.start('comecao')});
